...
_normalization_dict: Dict[Tuple[str, Union[str, Callable]],
                          NormalizationInfo] = {}
...
def register_acc_op_mapping(
    ...
    def insert(new_fn_target: Callable):
        ...
        if arg_replacement_tuples is None:
            final_arg_replacement_tuples = _get_dup_signature_tuples(
                new_fn_target)
        else:
...
def register_custom_acc_mapper_fn(
    ...
    def insert(custom_mapping_fn: Callable):
        _insert_fun(
            ...
            custom_mapping_fn=custom_mapping_fn,
            arg_replacement_tuples=
            arg_replacement_tuples,  # type: ignore[arg-type]
            needs_shapes_for_normalization=needs_shapes_for_normalization,
            allow_normalize_from_torch_package=
            allow_normalize_from_torch_package,
        )
...
def get_normalized_kwargs(node: torch.fx.Node,
                          arg_replacement_tuples: ArgReplacementTuplesType):
    new_kwargs = {}
