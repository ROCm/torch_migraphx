...
def remove_duplicate_output_args(top_level: fx.GraphModule,
...
def _ensure_proper_output_use(user: fx.Node, target_node: fx.Node) -> int:
    ...
    assert (
        ...
        and len(user.args) == 2 and isinstance(user.args[0], fx.Node)
...
def _remove_duplicate_output_args(gm: fx.GraphModule) -> RemoveDuplicateResult:
    ...
    if not (isinstance(args, t.Iterable)
            and all(isinstance(a, fx.Node) for a in args)):
    ...
    for idx, a in enumerate(args):
        assert isinstance(
            a, fx.Node), f"Expecting fx.Node instance, but got: {type(a)}"
        ...
        if a.name not in name_to_idx:
            ...
            changed = True
            _LOGGER.warning(f"Replaced duplicate output arg '{a.name}': "
