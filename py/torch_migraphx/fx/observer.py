...
TObserverCallback = t.TypeVar("TObserverCallback",
...
class Observer(t.Generic[TObserverCallback]):
    ...
    def observe(self, *args, **kwargs) -> None:
        for callback in self._get_callbacks():
            with _log_error("Error calling observer callback",
                            rethrow=RETHROW_CALLBACK_EXCEPTION):
...
def _log_error(msg: str, rethrow: bool = False) -> t.ContextManager:
    try:
        ...
        _e = e  # noqa: F841
        _LOGGER.info(
            f"{msg} (This error is handled): {traceback.format_exc()}")
        if rethrow:
            raise